...
<script>
import {Icon,Group,InlineDesc} from 'vux'
import Picker from 'components/datetime/datetimepicker'
import Uuid from 'vux/src/mixins/uuid'
...
export default {
  ...
  props: {
    ...
    startDate: {
      ...
      validator (val) {
        /* istanbul ignore if */
        if (process.env.NODE_ENV === 'development' && val && val.length !== 19) {
          console.error('[VUX] Datetime prop:start-date 必须为 YYYY-MM-DD HH:mm:ss 格式')
        }
        return val ? val.length === 19 : true
      }
    },
    endDate: {
      type: String,
      validator (val) {
        /* istanbul ignore if */
        if (process.env.NODE_ENV === 'development' && val && val.length !== 19) {
          console.error('[VUX] Datetime prop:end-date 必须为 YYYY-MM-DD HH:mm:ss 格式')
        }
        return val ? val.length === 19 : true
      }
    },
